...
proc day2*(fileName: string, partFn: (int, openarray[string]) -> int): int =
    ...
    for line in lines(fileName):
        ...
        # echo(gameSplit[0].split(" "))
        let gameId = parseInt(gameSplit[0].split(" ")[1])
        # echo("game:", gameId, gam)
        let gameSet = game.split(";")
        # echo "==="
        # echo line
        sum += partFn(gameId, gameSet)
    return sum
    ...
    for gs in gameSet:
        ...
        for cube in cubes:
            ...
            let
                ...
                color = cs[2]
            if color == "red":
                red = if (red == -1): count else: max(red, count)
            elif color == "green":
                green = if (green == -1): count else: max(green, count)
            elif color == "blue":
                blue = if (blue == -1): count else: max(blue, count)
