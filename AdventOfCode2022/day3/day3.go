...
func itemIntersects(mf map[rune]bool, items []map[rune]bool) rune {
    for item, _ := range mf {
        count_intersect := 0
        for _, f := range items {
            if f[item] {
...
func part2() int {
    ...
    onLine := func(line string) error {
        if line_count == 3 {
            item := itemIntersects(items[minItemsIdx], append(items[:minItemsIdx], items[minItemsIdx:]...))
            item_priority := itemPriority(item)
            // fmt.Println(string(item), item_priority)
            sum_priority += item_priority
            line_count = 0
            minItemsIdx = 0
            items = []map[rune]bool{}
        }
        if len(line) > 0 {
            items = append(items, itemsCount(line))
            if len(items[minItemsIdx]) > len(items[len(items) - 1]) {
                minItemsIdx = len(items) - 1
            }
        }
        line_count += 1
        return nil
